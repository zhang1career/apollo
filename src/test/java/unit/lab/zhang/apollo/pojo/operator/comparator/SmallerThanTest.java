...
import lab.zhang.apollo.pojo.operator.comparator.SmallerThan;
...
public class SmallerThanTest {
    ...
    public void test_calc() {
        ...
        assertFalse(target.calc(Lists.list(op1, op0), paramContext, exeConfig));
        assertTrue(target.calc(Lists.list(op2, op3), paramContext, exeConfig));
        assertFalse(target.calc(Lists.list(op3, op2), paramContext, exeConfig));

        assertTrue(target.calc(Lists.list(op0, op2), paramContext, exeConfig));
        assertFalse(target.calc(Lists.list(op3, op1), paramContext, exeConfig));
    }
